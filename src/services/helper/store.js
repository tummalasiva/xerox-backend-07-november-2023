...
module.exports = class StoreHelper {
  static async create(body, userId) {
    try {
      ...
      if (stores && stores._id) {
        await systemUsersData.updateOneUser(
          { _id: ObjectId(userId) },
          { $push: { store: stores._id } }
        );
  ...
  static async list(params) {
    try {
      ...
      let userInfo = await systemUsersData.findOne({
        _id: params.decodedToken._id,
      });
  ...
  static async update(id, body, userId) {
    try {
      let existDoc = await storesData.findOne({
        name: body.name,
        _id: { $ne: id },
      });
  ...
  static async delete(id, userId) {
    try {
      let stores = await storesData.updateOneStore(
        { _id: id },
