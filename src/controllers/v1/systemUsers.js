...
module.exports = class SystemUsers {
	...
	async create(req) {
		...
		try {
			const createdAccount = await systemUsersHelper.create(params,req.decodedToken._id)
	...
	async update(req) {
		...
		try {
		  const result = await systemUsersHelper.update(_id,params,req.decodedToken._id);
		  return result;
		} catch (error) {
		  return error;
		}
