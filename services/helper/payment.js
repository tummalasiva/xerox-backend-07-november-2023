    ...
    try {
      ...
      if (expectedSignature === params.razorpay_signature) {
        ...
        let paymentInfo = await paymentData.findOne({
          paymentId: params.razorpay_order_id,
        });

        let paymentUpdate = await paymentData.update(
          { paymentId: params.razorpay_order_id },
          { paidAt: new Date(), isPaid: true }
        );

        let orderInfo = await orderData.findOne({
          _id: ObjectId(paymentInfo.orderId),
        });

        let orderUpdate = await orderData.updateOneOrder(
        ...
        if (userInfo) {
          let smsInfo2 = await notifications.sendSms({
            to: userInfo.mobile,
            message: utilsHelper.composeEmailBody(
              common.ORDER_COMPLETE_MESSAGE,
              {
                name: userInfo.name,
        ...
        let employees = await systemUsersData.findEmployees({
          store: orderInfo.storeId,
        });
        ...
        await notifications.sendPushNotification(tokens, "New Order", {
          orderId: orderInfo._id.toHexString(),
